...
- 对象定位：搜索太阳、月球、八大行星或重点小天体，只会选中对象并显示方向和距离，不会移动相机。
...
历史日期会同时加载相邻两天并按 UTC 日内进度平滑混合。地球外层叠加低透明度的真实云图，并进行缓慢漂移和轻微形变；这是“动态云演示”，不代表实时风场或天气预报。状态栏会持续显示实际分辨率、观测日期、延迟、数据源和回退状态。
