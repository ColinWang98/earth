...
首屏直接使用随站点发布的 `2026-08-15` NASA GIBS VIIRS/Suomi NPP 真彩快照（桌面 4K、Quest/移动端 2K），不会自动请求远程影像。点击“更新卫星影像”后，页面才会探测当天及之前三天并选择最新覆盖较完整的全球影像；高性能桌面近景会在显卡、设备内存和帧时间允许时加载 8192×4096，普通桌面加载 4096×2048，Quest/移动端加载 2048×1024。8K 请求或性能不满足要求时自动退回 4K。
...
- 地球背景星空使用轻量 HYG 亮星表，并按星等调整点径与亮度；桌面绘制 1,250 颗，移动端绘制 850 颗。本版本不包含 Gaia 恒星邻域功能。
- 手机端保留机位、标签、小天体和详情四项紧凑控制；详情默认收起，时间控制使用双行布局，避免遮住主要观察区域。
